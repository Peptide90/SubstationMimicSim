...
function migrateScenario(scenario: ScenarioDefinition): ScenarioDefinition {
  return {
    ...
    maxTier: scenario.maxTier,
    recommendedTier: scenario.recommendedTier ?? 'Apprentice',
    supportedTiers: scenario.supportedTiers,
    explanationVariants: scenario.explanationVariants ?? {},
    ...
    teachingSteps: scenario.teachingSteps ?? [],
    currentStepIndex: scenario.currentStepIndex ?? 0,
    scoring: scenario.scoring ?? { stars: 0, score: 0, operationCount: 0, penalties: 0 },
    ...
    objectives: (scenario.objectives ?? []).map((objective) => ({ type: 'operate-switchgear', tierText: {}, tierHints: {}, ...objective })),
    tierObjectives: scenario.tierObjectives ?? {},
