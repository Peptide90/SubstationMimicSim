...
export function symbolGlyphSvg(symbol: ElectricalSymbol): string {
  ...
  if (symbol.type === 'transformer') return `<circle cx="-9" cy="0" r="13" fill="none" stroke="${selectedStroke}" stroke-width="2"/><circle cx="9" cy="0" r="13" fill="none" stroke="${selectedStroke}" stroke-width="2"/>`;
  if (symbol.type === 'ct') return `<line x1="-${SWITCH_TERMINAL_SPAN}" y1="0" x2="${SWITCH_TERMINAL_SPAN}" y2="0" stroke="${selectedStroke}" stroke-width="2"/><circle cx="-8" cy="0" r="11" fill="none" stroke="${selectedStroke}" stroke-width="2"/><circle cx="8" cy="0" r="11" fill="none" stroke="${selectedStroke}" stroke-width="2"/>`;
  if (symbol.type === 'vt') return `<line x1="0" y1="-26" x2="0" y2="-8" stroke="${selectedStroke}" stroke-width="2"/><circle cx="0" cy="5" r="13" fill="none" stroke="${selectedStroke}" stroke-width="2"/><text x="0" y="9" text-anchor="middle" font-size="10" fill="${selectedStroke}">V</text><line x1="0" y1="18" x2="0" y2="28" stroke="${selectedStroke}" stroke-width="2"/><line x1="-9" y1="28" x2="9" y2="28" stroke="${selectedStroke}" stroke-width="2"/><line x1="-6" y1="32" x2="6" y2="32" stroke="${selectedStroke}" stroke-width="2"/><line x1="-3" y1="36" x2="3" y2="36" stroke="${selectedStroke}" stroke-width="2"/>`;
