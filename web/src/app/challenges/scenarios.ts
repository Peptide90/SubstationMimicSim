...
export const CHALLENGE_SCENARIOS: ChallengeScenario[] = [
  ...
  },
  {
    ...
    title: "Level 4: Power Flow Switching",
    type: "level",
    difficulty: 4,
    description: "Learn how disconnectors, breakers, and earth switches change power-flow paths.",
    enablePowerSim: true,
    initialGraph: {
      locked: {
        nodes: [
          { ...makeNode("iface", "P1-SRC", 40, 220, { label: "Source", locked: true, iface: { substationId: "SUB", terminalId: "P1S" } }), data: { ...(makeNode("iface", "P1-SRC", 40, 220, { label: "Source", locked: true, iface: { substationId: "SUB", terminalId: "P1S" } }).data as any), power: { role: "source", pMw: 160, qMvar: 10 } } },
          { ...makeNode("iface", "P1-LOAD", 960, 220, { label: "Load", locked: true, iface: { substationId: "SUB", terminalId: "P1L" } }), data: { ...(makeNode("iface", "P1-LOAD", 960, 220, { label: "Load", locked: true, iface: { substationId: "SUB", terminalId: "P1L" } }).data as any), power: { role: "load", pMw: 150, qMvar: 65 } } },
          makeNode("junction", "P1-JL", 180, 220, {}),
          makeNode("junction", "P1-JR", 820, 220, {}),
          makeNode("ds", "P1-DS-A1", 300, 120, { state: "open" }),
          makeNode("cb", "P1-CB-A", 500, 120, { state: "open" }),
          makeNode("ds", "P1-DS-A2", 700, 120, { state: "open" }),
          makeNode("ds", "P1-DS-B1", 300, 320, { state: "open" }),
          makeNode("cb", "P1-CB-B", 500, 320, { state: "open" }),
          makeNode("ds", "P1-DS-B2", 700, 320, { state: "open" }),
          makeNode("es", "P1-ES-A", 500, 440, { state: "open" }),
        ],
        edges: [
    ...
    tutorialSteps: [
      { id: "p1-stage-0a", title: "Prepare the path", body: "Close DS-A1 and DS-A2 first. Disconnectors make an isolating path but should not be used to interrupt load.", requires: { type: "toggle", params: { nodeIds: ["P1-DS-A1", "P1-DS-A2"], to: "closed" } } },
      { id: "p1-stage-0b", title: "Energize with the breaker", body: "Close CB-A to energize the load. The breaker is the device designed to make and break load current.", requires: { type: "toggle", params: { nodeId: "P1-CB-A", to: "closed" } } },
      { id: "p1-stage-1", title: "Share the load", body: "Close DS-B1, DS-B2, then CB-B to create a second path and split the power flow.", requires: { type: "toggle", params: { nodeIds: ["P1-DS-B1", "P1-CB-B", "P1-DS-B2"], to: "closed" } } },
      { id: "p1-stage-2", title: "Faulted circuit", body: "A fault appears on circuit A. Open CB-A first so the breaker clears the current while circuit B keeps the load supplied.", requires: { type: "toggle", params: { nodeId: "P1-CB-A", to: "open" } } },
      { id: "p1-stage-3a", title: "Isolate for maintenance", body: "With CB-A open, open DS-A1 and DS-A2 to create visible isolation around the faulted circuit.", requires: { type: "toggle", params: { nodeIds: ["P1-DS-A1", "P1-DS-A2"], to: "open" } } },
      { id: "p1-stage-3b", title: "Apply earth", body: "Close ES-A only after the circuit is isolated, proving the difference between isolation and earthing.", requires: { type: "toggle", params: { nodeId: "P1-ES-A", to: "closed" } } },
    ],
    challengePhases: {
